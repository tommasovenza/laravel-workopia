...
<div class="rounded-lg shadow-md bg-white p-4 mb-4">
    <div class="flex items-center space-between gap-4">
        @if ($job->company_logo)
            <img src="/storage/{{ $job->company_logo }}" alt="{{ $job->title }}" class="w-14"/>
        @endif
        <div>
    ...
    <a href="{{ route('show', $job->id) }}" class="block w-full text-center px-5 py-2.5 shadow-sm rounded border text-base font-medium text-indigo-700 bg-indigo-100 hover:bg-indigo-200">
        Details
    </a>
</div>
